...
let procesosSuspendidos =[];
let listaSuspendidos = [];
...
function addEventListener(){
    ...
    document.addEventListener('keydown', (e)=>{
        ...
        switch(e.key){
            ...
            case 's':
                alerta.innerHTML = `SUSPENDIDO`;
                if (!pause && procesosBloqueados.length > 0) {
                    const procesoSuspendido = procesosBloqueados.shift();
                    suspenderProceso(procesoSuspendido);
                }
                break;
            case 'r':
                if (!pause) {
                    const procesoSuspendido = procesosSuspendidos.shift();
                    if (procesoSuspendido) { 
                        if (memory.freeFrames.length >= procesoSuspendido.pages) { 
                            procesoSuspendido.estado = estados.LISTO; 
                            procesosNuevos.unshift(procesoSuspendido); 
                        } 
                    } 
                }
                break;
...
async function run(){
    ...
    const crearArchivoBtn = document.getElementById('crearArchivoBtn');
    crearArchivoBtn.style.display = 'block';
...
function suspenderProceso(proceso) {
    if (proceso.estado === estados.BLOQUEADO) {
        proceso.servicio = 0;
        proceso.estado = estados.SUSPENDIDO;
        procesosSuspendidos.push(proceso);
        listaSuspendidos.push(proceso.id);
        memory.remove(proceso.id);
        procesosBloqueados = procesosBloqueados.filter(p => p !== proceso);
    }
}
...
async function manageProcess(){
    return new Promise((resolve)=>{
        ...
        .action(timer=>{
            ...
            let tamanoPrimerProceso = (procesosNuevos.length > 0) ? procesosNuevos[0].size : 0;
            document.querySelector('#tamanoNuevos').textContent = ` Tamaño del primer proceso: ${tamanoPrimerProceso}`;
...
function isLastProceso(){
    return !procesosNuevos.length && !procesosListos.length && !procesosBloqueados.length && !procesosSuspendidos.length;
}
...
document.addEventListener('DOMContentLoaded', function() {
    const crearArchivoBtn = document.getElementById('crearArchivoBtn');
    crearArchivoBtn.addEventListener('click', function() {
      const contenidoArchivo = listaSuspendidos.join('\n');
      const nombreArchivo = 'suspendidos.txt';
      descargarArchivo(contenidoArchivo, nombreArchivo);
    });
