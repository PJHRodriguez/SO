...
function actualizarPoceso(){
    ...
    if(procesoActual == PROCESO_NULL && procesosBloqueados.length){
        ...
        `;
    }else if(procesoActual == PROCESO_NULL && procesosSuspendidos.length){
        divProceso.innerHTML = `
           <p>ID: <span>NULL</span></p>
           <p>Operacion: <span>NULL</span></p>
           <p>Tiempo Max. Estimado: <span>NULL</span></p>
...
function createFinalTable(){
    ...
    const tablaListos = document.querySelector('#loteProceso');
...
function actualizarSuspendidos(){
    const tablaSuspendido = document.querySelector('#Suspendidos');
    tablaSuspendido.innerHTML = ` `;
    
    procesosSuspendidos.forEach(process=>{
        const row = document.createElement('TR');
        row.innerHTML = ` 
            <td>${process.id}</td>
        `;
        tablaSuspendido.appendChild(row);
    })
}
