...
function sacarDeNuevos() {
    if (isSacarNuevos()) {
        const nprocesos = Number(document.querySelector("#procesos").value);
        let process = procesosNuevos.shift();

        // Verificar si el número total de procesos es mayor a 4 o si el número actual de procesos sacados de la lista es mayor a 4
        if (numeroProcesos <= 4 && nprocesos > 3) {
            process.llegada = 0; // Establecer tiempo de llegada en 0 para los primeros 4 procesos
        } else {
            // Para los procesos adicionales o cuando el número total de procesos es menor o igual a 4, usar el tiempo del cronómetro global
